...
export default function CustomerLedgerPage() {
  ...
  const fetchLedger = async () => {
    const res = await fetch(`/api/tenant/finance/ledger/customer/${id}?startDate=${startDate}&endDate=${endDate}`)
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)}
              className="px-4 py-2.5 text-sm rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" />
            <button onClick={fetchLedger} className="px-5 py-2.5 text-sm font-semibold text-white rounded-xl"
              style={{ background: "linear-gradient(135deg, #2563EB, #0EA5E9)", boxShadow: "0 4px 16px rgba(37,99,235,0.25)" }}>Filter</button>
          </div>
        </div>
